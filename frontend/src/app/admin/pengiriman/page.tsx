...
const FALLBACK: OrderShip[] = [
  { id: "ORD-001", uuid: "ORD-001", buyer: "Siti Rahmawati", storeName: "Batik Kawung Jaya", product: "Kain Batik Tulis Motif Mega Mendung", qty: 1, address: "Jl. Merdeka No. 12, Bandung, Jawa Barat 40111", date: "28 Okt 2023", status: "Perlu Dikirim" },
  { id: "ORD-002", uuid: "ORD-002", buyer: "Budi Santoso", storeName: "Mitra Rotan Abadi", product: "Mangkuk Keramik Handmade Motif Tradisional", qty: 2, address: "Jl. Sudirman No. 45, Jakarta Selatan 12190", date: "27 Okt 2023", status: "Perlu Dikirim" },
  { id: "ORD-003", uuid: "ORD-003", buyer: "Dewi Kusuma", storeName: "Cokelat Nusantara", product: "Kopi Arabika Gayo Single Origin 250g", qty: 3, address: "Jl. Diponegoro No. 8, Surabaya, Jawa Timur", date: "26 Okt 2023", status: "Sedang Dikirim", courier: "JNE", resi: "JNE123456789", eta: "30 Okt 2023" },
  { id: "ORD-004", uuid: "ORD-004", buyer: "Reza Pratama", storeName: "Sutra Kawung Jaya", product: "Dompet Kulit Sapi Asli Cognac Brown", qty: 1, address: "Jl. Gatot Subroto No. 22, Medan, Sumatera Utara", date: "25 Okt 2023", status: "Sedang Dikirim", courier: "SiCepat", resi: "SC987654321", eta: "29 Okt 2023" },
  { id: "ORD-005", uuid: "ORD-005", buyer: "Anita Wahyuni", storeName: "Batik Kawung Jaya", product: "Paket Skincare Alami Ekstrak Kunyit", qty: 1, address: "Jl. A. Yani No. 5, Yogyakarta 55000", date: "20 Okt 2023", status: "Selesai", courier: "J&T Express", resi: "JT112233445", eta: "23 Okt 2023" },
];

function mapShipStatus(statOrder: string, statKirim: string | undefined): ShipStatus {
...
export default function AdminPengirimanPage() {
  const [orders, setOrders] = useState<OrderShip[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<ShipStatus>("Perlu Dikirim");
  const [modalOrder, setModalOrder] = useState<OrderShip | null>(null);
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch orders that are NOT cancelled and NOT just pending-unpaid
      const { data, error } = await supabase
        ...
        .select(`
          ...
          users ( nama_lengkap ),
          seller ( nm_store ),
          alamat ( detail_alamat, kota, provinsi ),
          order_item (
            qty_orderitem,
            produk ( nama_produk )
          ),
      ...
      const mapped: OrderShip[] = data
        .filter((o: any) => {
          // Only show orders that have been paid (diproses, dikirim, selesai) or have items
        ...
        .map((o: any) => {
          ...
          const addressText = alamat ? `${alamat.detail_alamat}, ${alamat.kota}, ${alamat.provinsi}` : "Alamat tidak tersedia";
          const shipStatus = mapShipStatus(o.stat_order, pengiriman?.stat_kirim);

          return {
            id: o.id_order.substring(0, 8).toUpperCase(),
            uuid: o.id_order,
      ...
      setOrders(mapped.length > 0 ? mapped : FALLBACK);
    } catch {
      setOrders(FALLBACK);
    } finally {
      setLoading(false);
  ...
  function openModal(order: OrderShip) { setModalOrder(order); setCourier("JNE"); setResi(""); setEta(""); }
  ...
  async function submitResi(e: React.FormEvent) {
    ...
    try {
      ...
      await shippingService.createShipment(modalOrder.uuid, courier, resi);
      // Update order status
      await supabase.from("order").update({ stat_order: "dikirim" }).eq("id_order", modalOrder.uuid);
      // Optimistic update
      setOrders((prev) => prev.map((o) =>
        o.uuid === modalOrder.uuid
          ? { ...o, status: "Sedang Dikirim" as ShipStatus, courier, resi, eta }
          : o
      ));
      setModalOrder(null);
  ...
  return (
    <div className="space-y-8">
      <header className="flex items-end justify-between">
        ...
        <button onClick={fetchOrders} className="flex items-center gap-1.5 text-[#1D4ED8] text-xs font-bold hover:underline">
      ...
      <div className="bg-white border border-[#EAE5E0] rounded-lg p-1 flex gap-1 shadow-sm w-fit">
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab}
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-extrabold ${activeTab === tab ? "bg-white/20 text-white" : "bg-[#F5F3F0] text-[#5C5550]"}`}>
              {tabCounts[tab]}
            </span>
          </button>
        ))}
                ...
                <div className="flex-1 space-y-3 min-w-0">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[#1F1B18] font-medium">{order.product} <span className="text-[#8E8680]">× {order.qty}</span></p>
                    </div>
                  </div>
                  ...
                  {order.status !== "Perlu Dikirim" && order.courier && (
                    <div className="flex items-center gap-2 flex-wrap pt-2 border-t border-[#F5F3F0]">
                      ...
                      {order.eta && <span className="text-xs text-[#8E8680]">• Est. tiba: {order.eta}</span>}
                    </div>
                  )}
                </div>
                <div className="flex-shrink-0">
                  {order.status === "Perlu Dikirim" && (
                    <button onClick={() => openModal(order)} className="flex items-center gap-2 px-5 py-2.5 bg-[#1D4ED8] text-white font-bold text-xs rounded-lg hover:bg-blue-700 transition">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6">
              <div className="bg-[#F5F3F0]/60 rounded-lg p-4 mb-5 space-y-1">
                ...
                <p className="text-sm font-semibold text-[#1F1B18]">{modalOrder.product} × {modalOrder.qty}</p>
              ...
              <form onSubmit={submitResi} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" placeholder="Contoh: JNE123456789" value={resi} onChange={(e) => setResi(e.target.value)} required
                ...
                <div>
                  ...
                  <input type="text" placeholder="Contoh: 30 Okt 2026" value={eta} onChange={(e) => setEta(e.target.value)} required
                    className="w-full h-11 border border-[#D5CFC9] rounded-lg px-4 text-sm font-semibold text-[#1F1B18] outline-none focus:border-[#1D4ED8] transition bg-[#F5F3F0]" />
                </div>
                <button type="submit" disabled={submitting}
                  ...
                  {submitting ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Menyimpan...</>
